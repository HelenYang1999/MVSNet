...
def homo_warping(src_fea, src_proj, ref_proj, depth_values):
    ...
    # 不会生成梯度
    with torch.no_grad():
        # torch.matmul 两个张量之间的矩阵相乘
        proj = torch.matmul(src_proj, torch.inverse(ref_proj))
        rot = proj[:, :3, :3]  # [B,3,3]
        trans = proj[:, :3, 3:4]  # [B,3,1]
        # x.shape(128,160) y.shape(128,160)
        y, x = torch.meshgrid([torch.arange(0, height, dtype=torch.float32, device=src_fea.device),
                               torch.arange(0, width, dtype=torch.float32, device=src_fea.device)])
        y, x = y.contiguous(), x.contiguous()
        y, x = y.view(height * width), x.view(height * width) # (20480)
        xyz = torch.stack((x, y, torch.ones_like(x)))  # [3,H*W]即[3,20480]
    ...
    # grid_sample函数是提供一个input的Tensor，以及一个对应的flow-field网格(比如光流，体素流等)
    # 然后根据grid中每个位置提供的坐标信息(这里指input中pixel的坐标)，将input中对应位置的像素值填充到grid指定的位置，得到最终的输出。
    warped_src_fea = F.grid_sample(src_fea, grid.view(batch, num_depth * height, width, 2), mode='bilinear',
                                   padding_mode='zeros') # [B,3,256*128,160]
...
# 返回的depth 的维度是 [B,H,W] 即在深度的方向上做加权和，得到最后的深度图。
...
if __name__ == "__main__":
    ...
    dataset = MVSDataset("/home/yangqi/dataset/mvsnet/training_data/dtu/", '../lists/dtu/train.txt', 'train',
                         3, 256)
    dataloader = DataLoader(dataset, batch_size=4,drop_last=True) #batch_size=2，how many samples per batch to load
    item = next(iter(dataloader))
    # 图片总数为 79*49*7=27097，当drop_last=True时，batch_size=4，len=6774。当drop_last=False时，batch_size=4，len=6775
    # 当当drop_last=True时，batch_size=8，len=3387。当drop_last=False时，batch_size=8，len=3388
    len = len(dataloader) #图片总数drop_last=True6775*4,3388*8
    #将512×640的图像裁减为128×160，每隔4个像素取一个
    imgs = item["imgs"][:, :, :, ::4, ::4].cuda()
    proj_matrices = item["proj_matrices"].cuda()
    mask = item["mask"].cuda()
    depth = item["depth"].cuda()
    depth_values = item["depth_values"].cuda()
    #假设神经网络已经处理完成。unbind 拆除指定的维度，1表示第2维，原先是2×3×3×128×160，拆完变成三个Tensor的tuple，每个Tensor的维度是2×3×128×160
