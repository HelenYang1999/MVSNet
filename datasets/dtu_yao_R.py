...
class MVSDataset(Dataset):
    def __init__(self, datapath, listfile, mode, nviews, ndepths=192, interval_scale=1.06, **kwargs):
        ...
        self.listfile = listfile #训练集的文件夹列表
        self.mode = mode
        self.nviews = nviews #视角的个数，nviews=3表示一个参考视角，两个其他视角
        self.ndepths = ndepths #深度值的数量
        self.interval_scale = interval_scale #区间尺度
        ...
        for scan in scans:
            ...
            with open(os.path.join(self.datapath, pair_file)) as f:
                ...
                for view_idx in range(num_viewpoint):
                    ref_view = int(f.readline().rstrip())#rstrip() 删除string字符串末尾的指定字符，默认为空白符 [1::2] 从第2个元素起，步长为2取元素
                    src_views = [int(x) for x in f.readline().rstrip().split()[1::2]]
                    # light conditions 0-6
                    for light_idx in range(7):
                        metas.append((scan, light_idx, ref_view, src_views))
        print("dataset", self.mode, "metas:", len(metas)) #metas的size为 27097= 79*49*7 #79个scan，49个相机视角，7个光照条件
        return metas
    ...
    def read_cam_file(self, filename):
        ...
        depth_interval = float(lines[11].split()[1]) * self.interval_scale #todo interval_scale、depth_interval、depth_min该如何理解
        return intrinsics, extrinsics, depth_min, depth_interval

    def read_img(self, filename):
        img = Image.open(filename)
        # scale 0~255 to 0~1 归一化到0-1之间 shape (512,640,3)
    ...
    def __getitem__(self, idx):
        ...
        for i, vid in enumerate(view_ids):
            ...
            img_filename = os.path.join(self.datapath,
                                        'Rectified/{}_train/rect_{:0>3}_{}_r5000.png'.format(scan, vid + 1, light_idx)) #图像
            mask_filename = os.path.join(self.datapath, 'Depths/{}_train/depth_visual_{:0>4}.png'.format(scan, vid)) #前后景的掩膜
            depth_filename = os.path.join(self.datapath, 'Depths/{}_train/depth_map_{:0>4}.pfm'.format(scan, vid)) #深度图，不区分光照
            proj_mat_filename = os.path.join(self.datapath, 'Cameras/train/{:0>8}_cam.txt').format(vid) #相机参数

            imgs.append(self.read_img(img_filename))
            intrinsics, extrinsics, depth_min, depth_interval = self.read_cam_file(proj_mat_filename)
            # 将内参与外参相乘得到投影矩阵，外参是一个4x4 的矩阵，内参是一个3×3的矩阵
        ...
        intrinsics_matrices = np.stack(intrinsics_matrices)
