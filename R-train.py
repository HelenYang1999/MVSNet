...
from models import *
from models.rmvsnet import RMVSNet, r_mvsnet_loss
from utils import *
import gc
import sys
import datetime
...
parser.add_argument('--model', default='rmvsnet', help='select model')

parser.add_argument('--dataset', default='dtu_yao_R', help='select dataset')
...
parser.add_argument('--interval_scale', type=float, default=1.06, help='区间尺度')
...
# model, optimizer
# 这里1，2，3表示希望使用的GPU编号，如果默认从0开始，可以去掉。
os.environ['CUDA_VISIBLE_DEVICES'] = '1,2,3'
model = RMVSNet()
if args.mode in ["train", "test"]:
    model = nn.DataParallel(model)
model.cuda()
model_loss = r_mvsnet_loss
optimizer = optim.Adam(model.parameters(), lr=args.lr, betas=(0.9, 0.999), weight_decay=args.wd)

# load parameters
# 继续上次的训练，需要设置参数args.resume 和 args.logdir
start_epoch = 0
...
def train():
    ...
    lr_gamma = 1 / float(args.lrepochs.split(':')[1]) #每次减小1/2
    lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones, gamma=lr_gamma,
                                                        last_epoch=start_epoch - 1)

    for epoch_idx in range(start_epoch, args.epochs):
        print('Epoch {}:'.format(epoch_idx))
        #通常用在epoch里面对lr进行调整。scheduler的step_size表示scheduler.step()每调用step_size次，对应的学习率就会按照策略调整一次
        lr_scheduler.step()
        ...
        #     start_time = time.time()
        #     global_step = len(TestImgLoader) * epoch_idx + batch_idx  #todo TrainImgLoader是不是写错了
        #     do_summary = global_step % args.summary_freq == 0
...
def train_sample(sample, detailed_summary=False):
    ...
    depth_est, probs = model(sample_cuda["images"], sample_cuda["intrinsics"], sample_cuda["extrinsics"], sample_cuda["depth_start"], sample_cuda["depth_interval"],sample_cuda["depth_num"])
    # depth_est = outputs["depth"]

    loss = model_loss(probs, depth_gt, sample["depth_num"], sample["depth_start"], sample["depth_interval"], mask)
    loss.backward() #反向传播
